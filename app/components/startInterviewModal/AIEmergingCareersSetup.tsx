...
import {
  AIEmergingCareersData,
  AIEmergingCareersSetupProps,
  Occupation,
  OccupationsResponse,
  RecommendationsResponse,
} from "@/app/interfaces/emergingCareers";
...
export default function AIEmergingCareersSetup({
  ...
  const [recommendations, setRecommendations] =
    useState<AIEmergingCareersData | null>(null);
  ...
  // Only fetch occupations once on mount
  useEffect(() => {
    fetchOccupations(1, true);
  }, []); // Remove dependency on token and other variables
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      if (searchQuery !== "") {
        ...
        // Don't fetch again if search is empty, just show existing occupations
  ...
  // Handle recommendation selection without calling occupations API
  const handleRecommendationSelect = (occupationName: string) => {
    // Don't update searchQuery to avoid triggering search
    onFormDataChange({
      ...formData,
      selectedOccupation: occupationName,
    });
  ...
  return (
    <div>
      ...
      {/* AI Emerging Career Recommendations */}
      {recommendations && (
        <div className="mb-6">
          ...
          {/* Three columns layout matching the image */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* Top 3 Similar Career */}
            <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
              <h5 className="text-base font-semibold text-blue-600 mb-4">
                Top 3 Similar Career
              </h5>
              <div className="space-y-3">
                {recommendations.response.top_3_similar_ai_emerging_occupations?.map(
                  (occupation) => (
                    <label
                      key={occupation._id}
            ...
            <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
              ...
              <div className="space-y-3">
                {recommendations.response.top_3_advance_ai_emerging_occupations?.map(
                  (occupation) => (
                    <label
                      key={occupation._id}
                      className={`flex items-center space-x-3 cursor-pointer transition-all ${
            ...
            <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
              ...
              <div className="space-y-3">
                {recommendations.response.top_3_lateral_ai_emerging_occupations?.map(
                  (occupation) => (
                    <label
                      key={occupation._id}
                      className={`flex items-center space-x-3 cursor-pointer transition-all ${
